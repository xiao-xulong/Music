<template>
  <div>
    <header>
      ...
      <svg @click="clear" v-show="user_search_info.trim()!=''" viewBox="0 0 1024 1024">
        <path
    ...
    <div v-if="commit_flag">
      <song_list @click="play_music(item.id,item.name)" v-for="item in search_result" :key="item.id" :Name="item.name"
        :SingerName="item.artists[0].name"> </song_list>
    </div>
    <van-overlay :show="$store.state.loadingShow">
      <van-loading color="#0094ff" size="60px" />
    </van-overlay>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    play_music(id, name) {
      this.$store.dispatch('play_music', { id: id, name: name })
    }
  }
}
</script>
